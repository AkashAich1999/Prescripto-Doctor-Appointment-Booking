...
const MyProfile = () => {
  ...
  return (
    <div className="max-w-lg flex flex-col gap-2 text-sm items-center sm:items-start">
      <img className="w-36 rounded" src={userData.image} alt="" />
      {
        isEdit
        ? <input className="max-w-60 bg-gray-50 text-3xl font-medium mt-4" type="text" onChange={e => setUserData(prev => ({...prev, name:e.target.value}))} value={userData.name} />
        : <p className="text-neutral-800 text-3xl font-medium mt-4">{userData.name}</p>
      }

      <hr className="bg-zinc-400 h-[1px] border-none" />
      <div>
        <p className="mt-3 underline text-neutal-500">CONTACT INFORMATION</p>
      ...
      <div>
        <p className="mt-3 underline text-neutal-500">BASIC INFORMATION</p>
        <div className="grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700">
